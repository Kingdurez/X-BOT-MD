...
async (conn, mek, m, { from, q, reply }) => {
    try {
        const url = 'https://evilinsult.com/generate_insult.php?lang=en&type=json';  // API for random why
        const response = await axios.get(url);
        const insult = response.data.insult;

        const insultMessage = `
  *X-BOT-MD RANDOM INSULT*
...
`;
        ...
        return reply(insultMessage);
    } catch (e) {
        console.log(e);
        return reply("⚠️ An error occurred while fetching a insult. Please try again later🤕.");
    }
});
